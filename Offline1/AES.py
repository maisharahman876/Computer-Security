...
def gen_func(lst,n,round_const):
    ...
    last[0]=last[0]^round_const
    # for j in last:
    #         print(hex(j))
    return last


def key_expansion(lst):
    n=int(len(lst))
    ...
    for i in range(n,n+r):
        if i%n==0:
            w_last=gen_func(lst,n,round_const)
            round=round+1
            if round_const<128:
                round_const=(2*round_const)
            else:
                round_const=(2*round_const)^283
            for j in range(0,4):
                w_last[j]=w_last[j]^lst[i-n][j] 
        else:
            w_last=list()
            for j in range(0,4):
                w_last.append(lst[len(lst)-1][j]^lst[i-n][j])
